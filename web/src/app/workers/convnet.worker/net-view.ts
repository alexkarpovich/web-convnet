...
export class NetView {
    ...
    constructor(private data: any) {
        this.dist = new ImageData(this.width, this.height);
    }

    static putImageData(distImageData, srcImageData, dx, dy,
                        dirtyX = undefined, dirtyY = undefined, dirtyWidth = undefined, dirtyHeight = undefined) {
        let data = srcImageData.data;
        let height = srcImageData.height;
        let width = srcImageData.width;
        let distWidth = distImageData.width;
        ...
        for (let y = dirtyY; y < limitBottom; y++) {
            for (let x = dirtyX; x < limitRight; x++) {
                let pos = y * width + x;
                let distPos = (y + dy) * distWidth + (x + dx);

                distImageData.data[distPos * 4] = data[pos * 4];
                distImageData.data[distPos * 4 + 1] = data[pos * 4 + 1];
                distImageData.data[distPos * 4 + 2] = data[pos * 4 + 2];
                distImageData.data[distPos * 4 + 3] = data[pos * 4 + 3];
            }
        }
    }
    ...
    prepareImageData() {
        let inImgData = new ImageData(this.data.size[0], this.data.size[1]);
    ...
    processConvLayer(layer: any) {
        ...
        for (let k = 0; k < layer.inCount * layer.count; k++) {
            let imData = new ImageData(layer.outSize[0], layer.outSize[1]);
    ...
    processFCLayer(layer: any) {
        ...
        let xoffset = lineCount < layer.out.length ? 0 : Math.ceil((this.width - preSize) / 2);

        for (let i = 0, j = 0, k = 0; i < layer.out.length; i++, j++) {
            if (j >= lineCount) {
                j = 0;
                k++;
            }
            let x = j * (FC_SIZE + 1) + xoffset,
                color = Math.floor(layer.out[i] * 255);

            NetView.fillRect(this.dist, x, this.vpos + k * (FC_SIZE + 1), FC_SIZE, FC_SIZE, color);
        }
    ...
    processOutputLayer(layer: any) {
        ...
        let xoffset = lineCount < layer.out.length ? 0 : Math.ceil((this.width - layer.out.length * (OUT_SIZE + 1) - 1) / 2);
        ...
        for (let i = 0, j = 0, k = 0; i < layer.out.length; i++, j++) {
            ...
            NetView.fillRect(this.dist, x, this.vpos + k * (OUT_SIZE + 1), OUT_SIZE, OUT_SIZE, color);
        }
